"""Always-on visual cues from the RGB image (no weight download).

Used as the default visual prior.  CLIP remains optional via
``SHIELD_VFV_CLIP=1``.  Dummy 4×4 black images used by tests are ignored.
"""
...
def image_cue_scores(image: np.ndarray | None) -> dict[str, float]:
    """Heuristic SEM.* scores from a uint8 HWC RGB image."""
    ...
    sharp = float(np.mean((r > 160.0) & (g > 160.0) & (b < 80.0)))  # yellow-ish metal
    liquid = float(np.mean((b > 140.0) & (g > 100.0) & (r < 90.0)))
    # Skin-ish: used only as a weak human cue.
    human = float(
        np.mean((r > 90.0) & (r < 220.0) & (g > 40.0) & (g < 180.0) & (b > 30.0) & (b < 150.0) & (r > g) & (g > b))
    )
